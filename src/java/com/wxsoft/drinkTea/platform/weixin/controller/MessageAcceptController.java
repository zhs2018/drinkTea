...
import com.wxsoft.drinkTea.platform.weixin.service.RecommendFriendService;
...
public class MessageAcceptController extends BaseAction {
	...
	@Autowired
	private RecommendFriendService recommendFriendSerice;
	...
	private StringBuffer handleEvent(Map<String, String> map){
		...
		if("subscribe".equals(map.get("xml.Event"))){
			...
			if(map.get("xml.EventKey")!=null && map.get("xml.EventKey").contains("qrscene_")){
				...
				logger.info("获取sId为："+sId);
				try {
					Integer id = Integer.parseInt(sId);
					WebUser user = webUserMapper.selectByPrimaryKey(id);//发送二维码的人
					WebUser user2 = new WebUser();
					user2.setOpenId(map.get("xml.FromUserName"));//扫描二维码的人
					user2 = webUserMapper.selectBy(user2);
					Integer count = null;
					Integer ct = null;
					if(user != null){
						if(user2!=null){//两个用户都已经关注过
                             ct = recommendFriendSerice.saveUserAndRecommendFriend(user, user2);
