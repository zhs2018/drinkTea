...
$(document).ready(function(){
	...
	$("#keyword").focus(function(){
		$(this).parent(".search_input").css("background-color","#ffffff");
	});
	$("#keyword").blur(function(){
		$(this).parent(".search_input").css("background-color","#eeeeee");
	});
	$("#keyword").keyup(function(event){
		var _result = $("#frame_search");
		var searchLength = parseInt($(".search_result li").index())+1;
		...
		success: function(data){
			...
			if (data.flag) {
				...
				if (data.data.contacts.d) {
					...
					contactsLengthAll = data.data.contacts.n;
					if (contactsLengthAll<=10) {
						contactsLength = contactsLengthAll;
					}else{
						contactsLength = 10;
						contactsInfo = '<li class="contactsMore">当前仅显示'+contactsLengthAll+'个搜索结果中的10项</li>';
					}
					showSearchContacts(0, contactsLength);
					$(".search_contact").find('.search_result').append(contactsInfo);
				}else{
					$(".search_contact").hide();
				}
				if (data.data.groups.d) {
					...
					groupsLengthAll = data.data.groups.n;
					if (groupsLengthAll<=10) {
						groupLength = groupsLengthAll;
					}else{
						groupLength = 10;
						groupsInfo = '<li class="groupsMore">当前仅显示'+groupsLengthAll+'个搜索结果中的10项</li>';
					}
					showSearchGroups(0, groupLength);
...
function showSearchContacts(startI, endI){
	...
	for( var i = startI; i<endI; i++){
		...
		keyInfo = "";
		subKeyWordIndex = 0;
		if (contactsArray[i].company != undefined) {
			matchInfo = MK_ESCAPE.tohtmlStr(contactsArray[i].company);
		}
		if (contactsArray[i].email != undefined) {
			matchInfo = MK_ESCAPE.tohtmlStr(contactsArray[i].email);
		}
		if (contactsArray[i].content != undefined) {
			matchInfo = MK_ESCAPE.tohtmlStr(contactsArray[i].content);
		}
		...
		if (contactsArray[i].tag != undefined) {
			matchTag = MK_ESCAPE.tohtmlStr(contactsArray[i].tag);
		...
		if(contactsArray[i].name != undefined){
			matchName = MK_ESCAPE.tohtmlStr(contactsArray[i].name);
		...
		$(".search_contact").find('.search_result').append("<li contactSearchId = '"+contactsArray[i].id+"'><div class='search_img_contact'></div><p class='contactR_name'>"+matchName+"</p><p class='contactR_matchTag'>"+matchTag+"</p><p class='contactR_matchInfo'>"+matchInfo+"</p></li>");
...
function showSearchGroups(startI,endI){
	...
	for( var i = startI; i<endI; i++){
		...
		keyInfo = "";
		subKeyWordIndex = 0;
		if(groupsArray[i].name != undefined){
			matchGroup = MK_ESCAPE.tohtmlStr(groupsArray[i].name);
		...
		$(".search_group").find('.search_result').append("<li groupSearchId = '"+groupsArray[i].id+"'><div class='search_img_group'></div><p class='groupResult_name'>"+matchGroup+"</p><span class='groupResult_count'>"+groupsArray[i].count+"</span></li>");
	}
}
				...
				$(".popwin_messageFeedback_confirm").click(function(){
					if ($(this).hasClass('btn_blue')) {
						...
						if ( $(".tbox .message_name").val() != "" && $(".tbox .message_email").val() != "" && $(".tbox .messagesTous").val() != "") {
							var generateJSON = '{"form":{"title":"留言反馈","logo":"","subtitle":""},"component":[{"name":"basic_name","id":"com1","title":"姓名","required":false,"instruct":"","value":"'+$(".tbox .message_name").val()+'","size":"input medium"},{"name":"basic_email","id":"com2","title":"邮箱","required":false,"instruct":"","value":"'+$(".tbox .message_email").val()+'","size":"input medium"},{"name":"basic_mobile","id":"com4","title":"手机","required":false,"instruct":"","value":"'+$(".tbox .message_phone").val()+'","size":"input medium"},{"name":"id_multiple","id":"com3","title":"留言内容","required":false,"instruct":"","value":'+$.toJSON($(".tbox .messagesTous").val())+',"size":"textarea medium"}],"contact":{"NAME":"'+$(".tbox .message_name").val()+'","LEGEND":'+$.toJSON( encodeValue( ($(".tbox .message_name").val()||"") ) )+',"GENDER":"","EMAIL":["'+$(".tbox .message_email").val()+'"],"COMPANY":"","CITY":"","JOB":"","WEBSITE":"","ADDRESS":"","FAX":[],"MOBILE":["'+$(".tbox .message_phone").val()+'"],"PHONE":[],"QQ":[],"FLAGS":[{"N":"留言用户","L":"lyyh"}],"GROUPS":"102"},"sendMail":true,"sendToEmail":"service@mikecrm.com;zhuyanbin@mikecrm.com;lihui@mikecrm.com","sendToName":"麦客团队"}';
		...
		success: function(data){
			if(data.flag){
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						"DATA": "{\"FRID\":\""+id+"\",\"FIELD\":\"TEMPLATE\",\"VALUE\":"+$.toJSON('{"form":{"title":"空白表单","logo":"","subtitle":"拖动内容进行编辑"},"component":[]}')+"}"
					},
					dataType: 'JSON',
					complete: function(){
						location.href = "formTemplate.php?ID="+id;
					}
				});
