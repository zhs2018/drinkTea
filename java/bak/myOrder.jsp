    ...
    <div>
        ...
        <div class="my_order" style="display: block;">
            ...
            <div class="my_div_list margin_bottom" id="id_${vs.index+1 }">
                <div class="my_t">
                    <p class="my_t_left"><fmt:formatDate value="${order.orderTime}" pattern="yyyy-MM-dd HH:mm:ss" /><!-- 2016-09-30 09:45 --></p>
                    <p class="my_t_right">
                    <c:choose>
                ...
                <div class="cart_sp ">
                    ...
                    <div class="cart_xx">
                        ...
                        <div class="cart_cz1">
                            ...
                            <p class="number"><label>×1</label></p>
                        </div>
                    </div>
                </div>
                </c:forEach>
                <div class="my-tj">共${order.count}件商品 小计：<label>¥${order.price}</label></div>
                <div class="my_btn">
                     <c:choose>
                    <c:when test="${order.orderState ==20}"><div class=" payment_btn" onclick = "apply(${order.id})">立即支付</div></c:when>
                    <c:when test="${order.orderState ==30}"><div class=" payment_btn" onclick ="refund(${order.id})">申请退款</div></c:when>
                    <c:when test="${order.orderState ==40}"><div class=" payment_btn confirmReceipt" id="${order.id}" >确认收货</div></c:when>
                    <c:when test="${order.orderState ==50}"><div class=" payment_btn" onclick ="eval(${order.id})">立即评价</div></c:when>
                    </c:choose>
                    <c:if test="${order.orderState ==20}">
                  	<div class="cancelOrder" id ="${order.id}"> 取消订单</div></c:if>
                    <a href="javascript:void(0)" onclick="seeOrders(${order.id},${order.orderState})"> <div class="payment_btn">
                                                                                                          查看详情</div></a>
                </div>
            </div>
            </c:forEach>
...
function apply(id){
	alert(id);
	window.location.href = "${RESOURCEDOMAIN}/confirmOrder?id="+id;
}
...
function seeOrders(id,orderStatus){
	window.location.href="${RESOURCEDOMAIN}/shop/seeOrders/"+id+"/"+orderStatus;
}
...
function confirmReceipt(id){
   ...
   /*  function gotos(orderState){
    	window.location.href =
    		"${RESOURCEDOMAIN}/shop/status/"+ orderState ;
    } */
</script>
</body>
</html>