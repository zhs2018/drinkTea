    ...
    <div class="content">
        <div class="order_add" onclick ="myAdd(${mp.id})">
           <%--  <a href="${RESOURCEDOMAIN}/shop/myAddress"> --%>
                ...
                <p class="order_name" >收货人：<label >${obj.userName}
                 <input  type="text" hidden="true" value = "${obj.userName}" name="userName"/>
                 <input  type="text" hidden="true" value = "${obj.id}" name="addressId"/>
                </label></p>
                <p class="order_tel" >${obj.telPhone}
                 <input  type="number" hidden="true" value = "${obj.telPhone}" name="telPhone"/></p>
                <div class="" >收货地址：<label >${obj.addressDetails}</label>
               </div>
           <!--  </a> -->
            <img  class="con_add_img con_add_img_go" src="${RESOURCEDOMAIN}/image/weixin/go.png" alt=""/>
        </div>

        <div class="cart_sp margin_top">
            <img name="picture" src="${RESOURCEDOMAIN}/${mp.picture}" alt=""/>
            <div class="cart_xx">
                <p class="cart_t">品牌名字<input  type="text" hidden="true" value = "${mp.name}" name="goodName"/>${mp.name}<!-- 奢宠精致茶礼盒装乌龙茶 -->
                <input  type="text" hidden="true" value = "${mp.id}" name="goodId"/></p>
                <p class="consumption"><label id="nowPrice">${mp.nowPrice}</label></p>
                <p class="order-numb">×1</p>
            </div>
        </div>

        <div class="con_add margin_top"><!-- </i> -->
            <p class="consumption1">购买数量</p>
            <div class="product_js" style="margin-top: 0.87125rem">
                <div class="js_jian">-</div>
                <input class="js_input" style="background: #fff" type="number" name="count" id="count" value="1" onChange="change()"/>
                <div class="js_jia">+</div>
            </div>
        </div>

        <div class="personal_div_h" id="ps_Popup" style="border-top: 1px solid #ececec">
        ...
        <div class="order-Red">
            <div class="order-Red-input">
                <label>红包抵现</label>
                <input type="number" placeholder="输入红包金额" id="redMoney" name="redMoney" value="" onChange="result()">
                <input  type="text" hidden="true" value = "${money}" name="money" id = "money"/>
            </div>
            	<p class="order-Red-k">可使用红包：<label>¥
            	<c:if test="${money == null}">0.00</c:if>
            	<c:if test="${money != null}">
            		<fmt:formatNumber pattern="0.00"  maxFractionDigits="2" value="${money}"/>
            	</c:if>
             </label></p>
        </div>
        ...
        <div class="personal_div_h">
            ...
            <textarea style=" width: 16.875rem;" class="personal_div_h_txt" placeholder="选填：对本次交易的说明" name="customerMessage" ></textarea>
        </div>

        <div class="personal_div_h border0">
            <div class="cart_settlement_total" style="color: #333">共<span id = "hejicount">1</span>件商品 合计:<label>￥</label><label id="heji">${mp.nowPrice}</label></div>
        </div>

        <!--提交订单-->
        <div class="cart_settlement_div">
            <div class="cart_settlement_btn" onclick ="apply(${obj.id})">提交订单</div>
            <div class="cart_settlement_total" style="margin-left: 0.625rem">实付:<label id="shifu">￥${mp.nowPrice}</label>
            <input  type="text" hidden="true" value ="${mp.nowPrice}" name="price" id="price"/>
            </div>
        </div>
    </div>
<!--支付式弹窗-->
	...
	$.ajax({
		...
		success:function(data){
			if(data.result == 0){
				...
				WeiXinPay(data);
				/* setTimeout(function(){
					window.location.href = "${RESOURCEDOMAIN}/shop/paymentSuccess?id="+data.id;
				}, 1000); */
...
function myAdd(id){
	window.location.href="${RESOURCEDOMAIN}/shop/myAddress?mpID="+id;
}
