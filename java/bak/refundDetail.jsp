...
<div class="verticalScreen">
 ...
 <c:forEach items="${Products}" var="od">
   <div class="cart_sp margin_top">
        ...
        <div class="cart_xx">
            <p class="cart_t">${od.name}${od.introduce}</p>
    ...
    <div class="orderd-xq" style="border-bottom: 0.5rem solid #eee">
        ...
        <div><label>退款状态：</label><span>
        	<c:choose>
		       <c:when test="${Order.orderState ==60}">申请处理中</c:when>
		       <c:when test="${Order.orderState ==70}">申请未通过</c:when>
		       <c:when test="${Order.orderState ==80}">申请已通过</c:when>
        	</c:choose>
        </span></div>
    </div>
