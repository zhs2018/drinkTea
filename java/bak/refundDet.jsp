...
<c:forEach items="${refund}" var="re">
     <div class="cart_sp margin_top" style="margin-top: 0.5rem">
        ...
        <div class="cart_xx">
            <p class="cart_t">${re.manageProducts.name}${re.manageProducts.introduce}</p>
    ...
    <div class="orderd-xq" style="border-bottom: 0.5rem solid #eee">
        ...
        <div><label>退款状态：</label><span>
        	<c:choose>
		       <c:when test="${re.productOrder.orderState ==60}">申请处理中</c:when>
		       <c:when test="${re.productOrder.orderState ==70}">申请未通过</c:when>
		       <c:when test="${re.productOrder.orderState ==80}">申请已通过</c:when>
        	</c:choose>
        </span></div>
    </div>
