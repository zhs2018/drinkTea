(function(win, doc) {
	...
	function _dialog() {
		...
		that.arg = (arg0 && (typeof arg0 == "object")) ? arg0 : {
			"str" : arg0
		};
		that._mask = [];
	...
	/** * */
	win.dialog = function(str, args) {
		return new _dialog(args).config(args);
	};
	win.alert = function(str, args) {
		return new _alert(str, args).config(args).open();
	};
	win.confirm = function(str, args) {
		return new _confirm(str, args).config(args).open();
	};
	win.loading = function(str, args) {
		return new _loading(str, args).config(args).open();
	};
	win.tip = function(str, args) {
		return new _tip(str, args).config(args).open().delay();
	};
})(window, document);
