...
<div class="verticalScreen">
   <c:forEach items="${objList}" var="obj" varStatus="vs">
    <div class="m_add_div" >
      <a href="#" onclick="jump(${obj.id},${mpId})">
       <input type="text" id="Id" name="id" placeholder="" value="${obj.id}" hidden="true"/>
        <input type="text" id="mpId" name="mpId" placeholder="" value="${mpId}" hidden="true"/>
       ...
       <c:if test="${obj.status == 2}">
         <div class="m_m style" onclick ="change(${obj.userId},${obj.id})">
            	<label class='cart_settlement_q'> <span class="m_add_xz style"></span><label>默认收货地址</label></label>
            </div>
       </c:if>
       <c:if test="${obj.status != 2}">
         <div class="m_m" onclick ="change(${obj.userId},${obj.id})">
            ...
            <div class="m_div_bj">
                <p class="m_bj"><a  href="javascript:void(0)"  onclick="edit_Address('${obj.id}')">
                <img src="${RESOURCEDOMAIN}/image/weixin/add_bj.png" alt="编辑"/><label>编辑</label>
                </a></p>
                <p class="m_delet" ><img src="${RESOURCEDOMAIN}/image/weixin/delete.png" alt="删除"/><label>删除</label></p>
            </div>
        </div>
        </c:forEach>
    </div>
   ...
   <a href="${RESOURCEDOMAIN}/shop/add_Address"><div class="refund-btn">添加新地址</div></a>
...
function addAdress(){
	var mpId=$("#mpId").val();
	alert(mpId);
	 window.location.href="${RESOURCEDOMAIN}/shop/add_Address?mpId"+mpId;
}
 ...
 function change(uid,id){
  $.ajax({
	  url:"${RESOURCEDOMAIN}/shop/change_Address",
	  type:"post",
	  data:{
		  userId:uid,
		  id:id,
	  ...
	  success:function(data){
		  if(data.status==1){
			  setTimeout(function(){
					window.location.href = "${RESOURCEDOMAIN}/shop/add_Address";
				}, 1000);
...
function del_Address(){
	var id=$("#Id").val();
 	$.ajax({
		url:"${RESOURCEDOMAIN}/shop/del_Address",
		type:"post",
		data:{
			id:id
		},
		...
		success:function(data){
			if(data.status==1){
                window.location.href="${RESOURCEDOMAIN}/shop/myAddress";
		   }else if(data.status==0){
			   window.location.href="${RESOURCEDOMAIN}/shop/myAddress";
		}
		},
  ...
  function edit_Address(id){
      window.location.href="${RESOURCEDOMAIN}/shop/edit_Address/"+id;
  }
